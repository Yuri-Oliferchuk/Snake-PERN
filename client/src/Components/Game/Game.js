...
import { getHighScore, postResultToDatabase } from "../../Utils/connection";
...
class Game extends React.Component {
  ...
  onGameOver = async () => {
    postResultToDatabase(this.state.userName, this.state.score).then(() => {
      getHighScore().then((data) => this.setState({ highScore: data }));
    });
    ...
    return (
      <div className='board'>
        ...
        <div className='highScore'>
          <h2>High Score:</h2>
          <BestPlayers bestPlayers={this.state.highScore} />
        </div>
